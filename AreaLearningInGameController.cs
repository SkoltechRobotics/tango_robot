...
public class AreaLearningInGameController : MonoBehaviour, ITangoPose, ITangoEvent, ITangoDepth
{
    ...
    /// <summary>
    /// Variable to hold coordinates of a selected point in point cloud.
    /// </summary>
    private Vector3 m_pointInSpace;

    /// <summary>
    /// Variable to hold robot relative movement.
    /// </summary>
    private Vector3 m_relativeMovement;

    /// <summary>
    /// Variable to hold screen coordinates of the touch.
    /// </summary>
    private Vector2 screenTouchPosition;

    /// <summary>
    /// WWW object to hold process get request for sending data to robot
    /// </summary>
    private WWW www;
    ...
    public void OnGUI()
    {
        var upperCenterStyle = new GUIStyle(GUI.skin.label);
        upperCenterStyle.alignment = TextAnchor.LowerCenter;
        upperCenterStyle.fontSize = 30;
        ...
        GUI.Label(new Rect(0, 0, Screen.width, 50), "Robot Location: " + "X = " + m_robotPoint.z + " Y = " + m_robotPoint.x, upperCenterStyle);
        GUI.Label(new Rect(0, 50, Screen.width, 50), "Target Location: " + "X = " + m_targetPoint.z + " Y = " + m_targetPoint.x, upperCenterStyle);
        GUI.Label(new Rect(0, 100, Screen.width, 50), "Touch Position: " + "X = " + screenTouchPosition.x + " Y = " + screenTouchPosition.y, upperCenterStyle);
        GUI.Label(new Rect(0, 150, Screen.width, 50), "Point Location: " + "X = " + m_pointInSpace.z + " Y = " + m_pointInSpace.x, upperCenterStyle);

        if (GUI.Button(new Rect(Screen.width - 220, 120, 200, 80), "<size=30>Robot</size>"))
        {
            StartCoroutine(_WaitForTouch("Robot"));
        }
        if (GUI.Button(new Rect(Screen.width - 220, 220, 200, 80), "<size=30>Location</size>"))
        {
            StartCoroutine(_WaitForTouch("Target"));
        }
        if (GUI.Button(new Rect(Screen.width - 220, 320, 200, 80), "<size=30>Send</size>"))
        {
            m_relativeMovement = m_targetPoint - m_robotPoint;
            string url = "http://10.30.98.21:8000/set/?xy=" + m_relativeMovement.z.ToString() + "," + (-m_relativeMovement.x).ToString();
            www = new WWW(url);
            StartCoroutine(_WaitForRequest(www));
            return;
        }
    ...
    /// <summary>
    /// Wait for the next touch input and start _WaitForDepth coroutine with it.
    /// </summary>
    /// <returns>Coroutine IEnumerator.</returns>
    private IEnumerator _WaitForTouch(string button)
    {
        do
        {
            yield return null;
        ...
        StartCoroutine(_WaitForDepth(screenTouchPosition, button));
    ...
    /// <returns>Coroutine IEnumerator.</returns>
    private IEnumerator _WaitForDepth(Vector2 touchPosition, string button)
    {
        m_findPlaneWaitingForDepth = true;
        ...
        {
            // Index is valid
            m_pointInSpace = m_pointCloud.m_points[pointIndex];
            switch (button)
            {
                case "Robot":
                    m_robotPoint = m_pointInSpace;
                    break;
                case "Target":
                    m_targetPoint = m_pointInSpace;
                    break;
            }
        }
    }

    private IEnumerator _WaitForRequest(WWW www)
    {
        yield return www;
